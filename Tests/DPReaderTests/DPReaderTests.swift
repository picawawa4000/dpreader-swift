...
/// Creates a schema decoder with an explicitly selected pack format.
func makeTestingJSONDecoder(_ packFormat: Version) -> JSONDecoder {
    let decoder = JSONDecoder()
    decoder.setDPReaderVersioning(PackVersioning(supportedVersions: .exactly(packFormat), selectedVersion: packFormat))
    return decoder
}

/// Creates a schema encoder with an explicitly selected pack format.
func makeTestingJSONEncoder(_ packFormat: Version) -> JSONEncoder {
