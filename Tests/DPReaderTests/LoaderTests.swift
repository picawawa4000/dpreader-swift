...
@Test func testLoadingForNoiseSettings() async throws {
    ...
    #expect(router.preliminarySurfaceLevel.map { testConstant(densityFunction: $0, expectedValue: 9.0) } == true)
    #expect(router.initialDensityWithoutJaggedness == nil)
...
@Test func testBakingForNoises() async throws {
    ...
    let worldGenerator = try WorldGenerator(withWorldSeed: 3447, usingDataPacks: [dataPack], usingSettings: nil, buildSearchTrees: false)
